...
def main():
    ...
    val_genotypes_modified = read_genotypes(filtered_val_bed, "validation", 10)
    test_genotypes_modified = read_genotypes(filtered_test_bed, "test", 10)
    ...
    tuner = setup_tuner(args.epochs, nn_directory_path, nn_project_name, input_shape)
    ...
    save_model_parameter_file = os.path.join(args.output_dir, "best_MLP_hp_model_params")
    save_model_file = os.path.join(args.output_dir, "best_MLP_hp_model")
    ...
    loss_plot_figure_file = os.path.join(args.output_dir, "MLP_loss_vs_epochs.png")
    plot_training_history(history, loss_plot_figure_file)
    
    print("Saving best model...")
    save_trained_best_model_file = os.path.join(args.output_dir, "MLP_trained_best_model.keras")
    ...
    train_correlation_NN, train_mse_NN = calculate_metrics(Y_train_tensor, Y_train_pred)
    val_correlation_NN, val_mse_NN = calculate_metrics(Y_val_tensor, Y_val_pred)
    test_correlation_NN, test_mse_NN = calculate_metrics(Y_test_tensor, Y_test_pred)
    ...
    csv_file = os.path.join(args.output_dir, "MLP_results.csv")
    save_results_to_csv(
        csv_file,
