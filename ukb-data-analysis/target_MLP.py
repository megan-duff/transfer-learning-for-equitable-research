...
def subset_genotype_matrix(common_snps, bim_file, genotype_matrix):
    # Filter to RSIDs / IIDs
    top_3000_clump_snp_dataset = common_snps.loc[common_snps['P'].abs().nlargest(153000).index]
    snp_filter_rs_ids = top_3000_clump_snp_dataset.iloc[:, 1].tolist()
    # Grab rows in bim file that are in the clump SNPs list
    matching_rows = bim_file[bim_file.iloc[:,1].isin(snp_filter_rs_ids)].index.tolist()
    # Print # of rows found 
    print(f"Number of Clump SNPs : {len(matching_rows)}", flush = True)
    # Subset the PLINK files based on the location of the top k SNPs list
    filtered_genotype_matrix = genotype_matrix[:,matching_rows]
    return (
...
merged_train_phenotype_data = merged_train_data[['IID', f'Corrected_{pheno_name_adjust}']]
...
# Subset to top 3k
initial_clump_snp_dataset=pd.read_csv(f"/mnt/project/Genotype_Data/target_clump_snps/irish_whole_genome_maf_unrelated_pheno_50_1neg2.clumps", sep="\s+")
clump_snp_dataset=pd.read_csv(f"/mnt/project/Genotype_Data/continuous-{pheno_code}-both_sexes-irnt-formatted-pvalue-1eneg2.assoc", sep="\t")
bim_file=pd.read_csv(f"/mnt/project/Genotype_Data/{ancestry}/target_model_files/{ancestry}_whole_genome_maf_unrelated_clump_snps_1e-2_train.bim", sep="\t", header=None)

common_snps = clump_snp_dataset[clump_snp_dataset['SNP'].isin(initial_clump_snp_dataset['ID'])]

train_genotype_matrix=scaled_train_genotypes_subset
val_genotype_matrix=scaled_val_genotypes_subset
test_genotype_matrix=scaled_test_genotypes_subset

filtered_train_genotype_matrix, matching_rows = subset_genotype_matrix(common_snps, bim_file, train_genotype_matrix)
filtered_val_genotype_matrix, matching_rows = subset_genotype_matrix(common_snps, bim_file, val_genotype_matrix)
filtered_test_genotype_matrix, matching_rows = subset_genotype_matrix(common_snps, bim_file, test_genotype_matrix)

# Rename the variables for neural network training! 
X_train = filtered_train_genotype_matrix
Y_train = pd.DataFrame(merged_train_phenotype_data.iloc[:,1].values.reshape(-1, 1))
X_val = filtered_val_genotype_matrix
Y_val = pd.DataFrame(merged_val_phenotype_data.iloc[:,1].values.reshape(-1, 1))
X_test = filtered_test_genotype_matrix
