...
def subset_bed_bim_fam(snp_file, p_value_threshold, train_bim, train_bed, train_fam, val_bim, val_bed, val_fam, test_bim, test_bed, test_fam):
    ...
    dtype_dict = {0: str, 1: str, 7: str, 8: str, 9: str, 10: str, 11: str, 12: str, 13:str, 14:str, 15:str}
    ...
    snp_filter_dataset = snp_filter_dataset[snp_filter_dataset.iloc[:, 14] != 'P']
    ...
    snp_filter_dataset.iloc[:, 14] = snp_filter_dataset.iloc[:, 14].astype(float)
    ...
    linear_rows = snp_filter_dataset[snp_filter_dataset.iloc[:, 9] == 'ADD']
    # cut_off_rows = linear_rows[linear_rows.iloc[:, 11] < p_value_threshold]
    cut_off_rows = linear_rows[linear_rows.iloc[:, 14] < p_value_threshold]
...
def standardize_geno(train_af_file, matching_rows, train_genotypes_modified, val_genotypes_modified, test_genotypes_modified):
    ...
    train_freq_vector = np.array(pd.to_numeric(af_df_subset.iloc[:, 5], errors='coerce'))
